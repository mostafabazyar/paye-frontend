...
async function main() {
  ...
  // Create some profiles (listings) for these users
  const profilesData = [
    {
  ...
  const createdProfiles = [] as any[];

  for (const p of profilesData) {
    const profile = await prisma.profile.create({ data: p as any });
    createdProfiles.push(profile);
  }

  // Create a couple of requests
  await prisma.request.create({ data: { profileId: createdProfiles[0].id, requesterId: createdUsers[1].id, receiverId: createdProfiles[0].userId, status: 'PENDING' } as any });

  await prisma.request.create({ data: { profileId: createdProfiles[1].id, requesterId: createdUsers[2].id, receiverId: createdProfiles[1].userId, status: 'PENDING' } as any });
